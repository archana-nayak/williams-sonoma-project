...
public class AutoBasics {
	...
	public static boolean takeScreenShots(WebDriver driver, String location, String fileName, Logger logger) {
		File src = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
		try {
			FileUtils.copyFile(src, new File(location + fileName + ".png"));
			// If the file is created(if it is not in the catch clause, it is),
			// return true
			return true;
		} catch (IOException e) {
			logger.info("Sorry you had a problem taking the screen shot");
			return false;
		}
	}
